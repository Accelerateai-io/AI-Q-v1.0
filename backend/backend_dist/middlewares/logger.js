...
function encodePlainLogValue(s) {
...
function flattenMetaPlain(data, prefix = "") {
    const out = [];
    ...
    if (typeof data === "string" ||
        ...
        typeof data === "boolean") {
        if (prefix)
            out.push({ key: prefix, value: String(data) });
        return out;
    }
    ...
    if (data instanceof Date) {
        if (prefix)
            out.push({ key: prefix, value: data.toISOString() });
        return out;
    }
    if (Array.isArray(data)) {
        if (!prefix)
            return out;
        const primitive = data.every((item) => item === null ||
            item === undefined ||
    ...
    if (typeof data === "object") {
        try {
            ...
            if (prefix)
                out.push({ key: prefix, value: "[unserializable]" });
        }
    }
    return out;
}
const filePlainFormat = winston.format.combine(winston.format.timestamp({ format: fileDateTimeLocal }), winston.format.printf((info) => {
    ...
    const pairs = flattenMetaPlain(Object.keys(rest).length ? rest : undefined)
...
export function attachProcessErrorLogging() {
