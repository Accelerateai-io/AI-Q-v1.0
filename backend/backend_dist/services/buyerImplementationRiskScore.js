...
function extractVendorTrustScore(attestationRow) {
    if (!attestationRow)
        return 50;
    const report = attestationRow.generated_profile_report;
    const trustScore = report?.trustScore;
    const raw = trustScore?.overallScore ??
        report?.formula?.vendor_trust_score ??
        report?.vendor_trust_score;
    const n = Number(raw);
    return clamp01(Number.isFinite(n) ? n : 50);
}
function calculateOrgReadinessGap(buyerPayload) {
    let risk = 35;
    const digital = norm(buyerPayload.digitalMaturityLevel);
    if (digital.includes("high") || digital.includes("advanced"))
        risk -= 10;
    else if (digital.includes("medium"))
        risk -= 4;
    else if (digital.includes("low") || digital.includes("ad-hoc"))
        risk += 10;
    const governance = norm(buyerPayload.dataGovernanceMaturity);
    if (governance.includes("optimized") || governance.includes("managed"))
        risk -= 8;
    else if (governance.includes("basic"))
        risk += 4;
    else if (governance.includes("ad-hoc") || governance.includes("low"))
        risk += 10;
...
function calculateIntegrationRisk(buyerPayload) {
    let risk = 25;
    ...
    if (rollback.includes("no"))
        risk += 12;
    else if (rollback.includes("manual"))
        risk += 6;
    else
        risk -= 3;
...
export function buyerImplementationReadinessGradeFromScore(rawScore) {
    return interpret(rawScore).grade;
}
export function calculateBuyerImplementationRiskScore(buyerPayload, attestationRow, vendorName, productName) {
