...
import { loadFrameworkDatasets } from "./frameworkDatasetsLoader.js";
...
const EXPIRY_LINE = /valid\s*(?:until|through|to|from)|expir(?:es|y|ation)|certificate\s*(?:is\s*)?valid|period\s*(?:of\s*)?(?:validity|coverage)|report\s*period|audit\s*period|coverage\s*period|effective\s*(?:until|through)|remains\s*valid|due\s*for\s*renewal/i;
function normalizeToken(s) {
    return s.toLowerCase().replace(/[^a-z0-9]+/g, " ").trim();
}
function tokenizeText(s) {
    return new Set(normalizeToken(s)
        .split(/\s+/)
        .filter((t) => t.length >= 3));
}
function classifyDocument(category, fileName, text) {
    const hay = `${category} ${fileName} ${text.slice(0, 5000)}`.toLowerCase();
    if (/soc\s*2|soc2/.test(hay))
        return "SOC2";
    if (/iso\s*27001|iso-?27001/.test(hay))
        return "ISO27001";
    if (/nist|800-53|800-171|csf/.test(hay))
        return "NIST";
    if (/hipaa|phi|e-?phi|\bbaa\b|hitrust/.test(hay))
        return "HIPAA";
    if (/gdpr|privacy/.test(hay))
        return "GDPR";
    if (/pci|dss/.test(hay))
        return "PCI-DSS";
    if (/dora/.test(hay))
    ...
    if (/cmmc/.test(hay))
        return "CMMC";
    if (/certificat|attestation|audit|compliance/.test(hay))
        return "COMPLIANCE_EVIDENCE";
    return "GENERAL_COMPLIANCE_DOCUMENT";
}
function frameworkFitsClass(name, docClass) {
    const n = name.toLowerCase();
    if (docClass === "SOC2")
        return n.includes("soc2");
    if (docClass === "ISO27001")
        return n.includes("iso");
    if (docClass === "NIST")
        return n.includes("nist");
    if (docClass === "HIPAA")
        return n.includes("hipaa");
    if (docClass === "GDPR")
        return n.includes("gdpr");
    if (docClass === "PCI-DSS")
        return n.includes("pci");
    if (docClass === "DORA")
        return n.includes("dora");
    if (docClass === "CMMC")
        return n.includes("cmmc");
    return true;
}
const MAX_STORED_CONTROL_DESCRIPTION_LEN = 4000;
function mapToFrameworkControls(text, docClass) {
    ...
    for (const ds of frameworks) {
        ...
        if (!frameworkFitsClass(fName, docClass))
            continue;
        const scored = ds.controls
            .map((c) => {
            const tokens = tokenizeText(`${c.title ?? ""} ${c.description ?? ""} ${c.implementationGuidance ?? ""}`);
            let overlap = 0;
            for (const t of tokens)
                if (textTokens.has(t))
                    overlap++;
            const descRaw = String(c.description ?? c.implementationGuidance ?? "").trim();
            const description = descRaw.length > MAX_STORED_CONTROL_DESCRIPTION_LEN
...
function parseDateCandidate(raw) {
    ...
    // Month DD, YYYY
    const re = new RegExp(`\\b(${MONTHS})\\s+(\\d{1,2})(?:st|nd|rd|th)?,?\\s*(\\d{4})\\b`, "i");
    m = s.match(re);
    if (m) {
...
function extractDatesFromChunk(chunk) {
    const out = [];
    ...
    const monthAll = chunk.matchAll(new RegExp(`\\b(?:${MONTHS})\\s+\\d{1,2}(?:st|nd|rd|th)?,?\\s*\\d{4}\\b`, "gi"));
...
function isReasonableCertDate(d) {
 ...
 */
export function extractExpiryFromText(fullText) {
    const text = fullText.replace(/\r\n/g, "\n").slice(0, 600_000);
    const lines = text.split("\n");
    const keywordDates = [];
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i] ?? "";
        if (!EXPIRY_LINE.test(line))
            continue;
...
function collectComplianceFiles(documentUploads) {
    const out = [];
    const seen = new Set();
    if (!documentUploads || typeof documentUploads !== "object")
        return out;
    const addFile = (rawName, category) => {
        const base = path.basename(rawName.trim());
        if (!base || base === "." || base === "..")
            return;
        if (path.extname(base).toLowerCase() !== ".pdf")
            return;
        if (seen.has(base))
            return;
        seen.add(base);
 ...
 */
export async function parseAndStoreComplianceDocumentExpiries(attestationId, documentUploads) {
    ...
    for (const { fileName, category } of items) {
        ...
        try {
            ...
            const validation = frameworkMapping != null
                ? {
                    isValid: frameworkMapping.controls.length > 0,
                    reason: frameworkMapping.controls.length > 0
    ...
    try {
        await db
            ...
            .set({
            compliance_document_expiries: result,
