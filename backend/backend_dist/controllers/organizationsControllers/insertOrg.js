...
import { inviteCustomerOrganizationAdmin, } from "../../services/organizationAdminInvite.js";
const EMAIL_RE = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
async function cleanupNewOrganization(orgId) {
...
const insertOrganization = async (req, res) => {
    try {
        ...
        const organizationTypeRaw = String(req.body.organizationType ?? "")
            .trim()
            .toLowerCase();
        const adminEmailRaw = req.body.adminEmail != null ? String(req.body.adminEmail).trim() : "";
        if (!organizationName) {
            return res
                .status(400)
        ...
        const organizationType = organizationTypeRaw;
        const organizationDuplicates = await db
            .select()
            .from(createOrganization)
            .where(sql `LOWER(TRIM("organizationName")) = LOWER(${organizationName})`)
            .limit(1);
