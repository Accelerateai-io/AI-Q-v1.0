...
function parseCustomerOrgPlatformRole(raw) {
...
const fetchOrganizations = async (req, res) => {
    try {
        ...
        const orgIdsWithAdmin = new Set(adminRows.map((r) => r.organization_id).filter((id) => id != null));
        /** Prefer admin's vendor/buyer platform role for customer orgs (not AI EVAL). */
        const organizationTypeFromAdmin = new Map();
        for (const row of adminRows) {
            const orgId = row.organization_id;
            if (orgId == null || orgId === AI_EVAL_ORG_ID)
                continue;
            if (organizationTypeFromAdmin.has(orgId))
                continue;
            const kind = parseCustomerOrgPlatformRole(row.user_platform_role);
            if (kind)
                organizationTypeFromAdmin.set(orgId, kind);
        }
        const data = organizations.map((org) => {
            const fromAdmin = org.id !== AI_EVAL_ORG_ID ? organizationTypeFromAdmin.get(org.id) : undefined;
