...
function readBuyerAttestationIdFromBody(body) {
    const keys = [
        "vendorAttestationId",
...
function buildBuyerContextForReport(body) {
    const g = (k) => body[k];
    return {
...
async function persistVendorRiskReport(assessmentId, body, vendorName, productName) {
    try {
        ...
        const report = await generateBuyerVendorRiskReport(buildBuyerContextForReport(body), attestation, vendorName || "Vendor", productName || "Product");
...
const submitBuyerCotsAssessment = async (req, res) => {
    try {
        ...
        if (assessmentId) {
            ...
            await persistVendorRiskReport(assessmentId, body, String(payloadCots.vendor_name ?? ""), String(payloadCots.specific_product ?? ""));
        ...
        await persistVendorRiskReport(assessment.id, body, String(payloadCots.vendor_name ?? ""), String(payloadCots.specific_product ?? ""));
