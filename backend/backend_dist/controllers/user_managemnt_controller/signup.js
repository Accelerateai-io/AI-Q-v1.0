...
import { buildOnboardingEmailHtml, ONBOARDING_MAIL_PLATFORM_NAME, } from "../../email/onboardingEmailHtml.js";
...
const userSignup = async (req, res) => {
    ...
    try {
        ...
        const orgUsersWithRole = await db
            ...
            .from(usersTable)
            .where(and(eq(usersTable.organization_id, orgId), isNotNull(usersTable.user_platform_role), ne(usersTable.user_platform_role, "")))
            .limit(1);
        ...
        if (userNameTrimmed !== "") {
            const existingByUsername = await db
                ...
                .from(usersTable)
                .where(and(eq(usersTable.user_name, userNameTrimmed), ne(usersTable.id, existingRow.id)))
                .limit(1);
        ...
        const orgOnboardedUsers = await db
            .select({ id: usersTable.id })
            .from(usersTable)
            .where(and(eq(usersTable.organization_id, dbUser.organization_id), eq(usersTable.user_onboarding_completed, "true")))
            .limit(1);
        const isOrgOnboardingCompleted = orgOnboardedUsers.length > 0;
        // Resolve organization (name + buyer/vendor) for email link and JWT
        let orgDisplayName = "";
        let organizationTypeForOnboarding = "vendor";
        ...
        if (isAdminUser) {
            ...
            if (!isOrgOnboardingCompleted) {
                ...
                await transporter.sendMail({
                    ...
                    html: buildOnboardingEmailHtml(dbUser.user_name ?? "User", capitalizeFirstLetter(String(dbUser.role ?? "")), onboardingLink, capitalizeFirstLetter(String(orgDisplayName ?? ""))),
                });
