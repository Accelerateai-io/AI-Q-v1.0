...
import { buildInviteUserEmailHtml, INVITE_MAIL_PLATFORM_NAME, } from "../../email/inviteUserEmailHtml.js";
    ...
    try {
        ...
        if (String(role).toLowerCase().trim() === "admin") {
            const existingAdmin = await db
                ...
                .from(usersTable)
                .where(and(eq(usersTable.organization_id, orgIdNum), eq(usersTable.role, "admin")))
                .limit(1);
...
export const resendOnboardingLink = async (req, res) => {
    ...
    try {
        ...
        const [orgRow] = await db
            ...
            .where(eq(createOrganization.id, user.organization_id))
            .limit(1);
        const organizationTypeForJwt = String(orgRow?.organizationType ?? "").trim().toLowerCase() === "buyer"
            ? "buyer"
            : "vendor";
        ...
        }, secret, { expiresIn: ONBOARDING_LINK_EXPIRY_JWT });
