...
import { EMAIL_BRAND_PRIMARY, EMAIL_PAGE_BG, emailSignatureCheckmarkHtml, } from "../../email/emailBrand.js";
const SIGNUP_CONFIRM_PLATFORM_NAME = "AI-Q Platform";
/** HTML email for signup confirmation (invite or reactivation). */
function signupConfirmationEmailHtml(organizationName, role, confirmationLink) {
...
const updatesUsers = async (req, res) => {
    ...
    try {
        ...
        const validOnboardingStatuses = ["completed", "expired", "pending"];
        const raw = data.onboarding_status && String(data.onboarding_status).toLowerCase();
        const onboardingStatus = raw && validOnboardingStatuses.includes(raw) ? raw : undefined;
        const systemRoles = [
            "system admin",
            "system user",
            "system manager",
        ...
        const platformRoleToSet = isAiEval && systemRoles.includes(newRole)
            ? newRole
            : isAiEval
            ...
            if (secret && BASE_URL) {
                try {
                    ...
                    await transporter.sendMail({
                        ...
                        html: signupConfirmationEmailHtml(organizationNameForLog, data.role ?? "", confirmationLink),
                    });
                }
                catch (emailErr) {
