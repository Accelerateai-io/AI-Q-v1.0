...
function normalizeDocumentUploads(raw) {
    ...
    const aiGovernancePolicy = Array.isArray(o.aiGovernancePolicy)
        ? o.aiGovernancePolicy.filter((c) => typeof c === "string")
        : [];
 ...
 */
function getAllDocumentFileNames(docUpload) {
    const names = new Set();
    ...
    add(docUpload.evidenceTestingPolicy ?? []);
    add(docUpload.aiGovernancePolicy ?? []);
    return names;
}
/**
 * Delete from disk any files that were removed from document_uploads (only files under attestation dir).
 */
function deleteRemovedDocumentFiles(attestationId, removedNames) {
 ...
 */
async function generateAndStoreProfileReport(vendorData, formulaPayload, userId, organizationIdStr, attestationId) {
    try {
...
const submitVendorSelfAttestation = async (req, res) => {
    try {
        ...
        const documented_ai_governance_policy = get("documented_ai_governance_policy") != null
        ...
        if (newAttestation || !attestationId) {
            ...
            if (status === "COMPLETED" && insertedId) {
                ...
                reportPayload = await generateAndStoreProfileReport(vendorData, b, userId, organizationIdStr ?? null, insertedId);
                if (document_uploads) {
                    void parseAndStoreComplianceDocumentExpiries(insertedId, document_uploads).catch((err) => console.error("Compliance document expiry parse:", err));
                }
            }
            const [rowAfter] = insertedId
                ? await db
                    ...
                    .limit(1)
                : [inserted];
            const att = rowAfter ? buildAttestationResponse(rowAfter) : (inserted ? buildAttestationResponse(inserted) : null);
            if (att && reportPayload)
                att.generated_profile_report = reportPayload;
            res.status(201).json({
                success: true,
        ...
        if (attestationId) {
            const updateWhere = organizationIdStr
                ? and(eq(vendorSelfAttestations.id, attestationId), or(eq(vendorSelfAttestations.user_id, userId), and(eq(vendorSelfAttestations.organization_id, organizationIdStr), eq(vendorSelfAttestations.status, "DRAFT"))))
            ...
            const oldDocUpload = existingById.document_uploads;
            const oldNames = getAllDocumentFileNames(oldDocUpload ?? null);
            const newNames = getAllDocumentFileNames(docUploadResult.value);
            ...
            if (status === "COMPLETED") {
                const vendorData = buildVendorDataFromPayload(b);
                reportPayload = await generateAndStoreProfileReport(vendorData, b, userId, organizationIdStr ?? null, attestationId);
                if (document_uploads) {
                    void parseAndStoreComplianceDocumentExpiries(attestationId, document_uploads).catch((err) => console.error("Compliance document expiry parse:", err));
                }
            }
            const [savedRow] = await db
                ...
                .limit(1);
            const att = savedRow ? buildAttestationResponse(savedRow) : null;
            if (att && reportPayload)
                att.generated_profile_report = reportPayload;
            res.status(200).json({
                success: true,
