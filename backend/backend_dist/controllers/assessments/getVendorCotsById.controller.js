...
import { buildVendorCotsOrganizationalPortalInsights, deriveBuyerIndustrySegmentForVendorCots, } from "../../services/orgPortalComplianceInsights.js";
...
const getVendorCotsById = async (req, res) => {
    try {
        ...
        const rows = await db
            ...
            .leftJoin(vendorSelfAttestations, or(eq(cotsVendorAssessments.vendor_attestation_id, vendorSelfAttestations.vendor_self_attestation_id), eq(cotsVendorAssessments.vendor_attestation_id, vendorSelfAttestations.id)))
        ...
        const data = {
            ...
            attestationProductName: r.attestation_product_name ?? "",
        ...
        const rx = r;
        const q = req.query;
        const segmentForInsights = deriveBuyerIndustrySegmentForVendorCots(data.customerSector, typeof q?.buyerIndustrySegment === "string" ? q.buyerIndustrySegment : undefined);
        ...
        let storedCustomerRiskReport;
        if (assessmentOrgId) {
            const [latestReport] = await db
                .select({ report: customerRiskAssessmentReports.report })
                .from(customerRiskAssessmentReports)
                .where(and(eq(customerRiskAssessmentReports.assessment_id, id), eq(customerRiskAssessmentReports.organization_id, assessmentOrgId)))
        ...
        data.organizationalPortal = buildVendorCotsOrganizationalPortalInsights(data, attestationRow, segmentForInsights, storedCustomerRiskReport !== undefined ? { storedCustomerRiskReport } : undefined);
