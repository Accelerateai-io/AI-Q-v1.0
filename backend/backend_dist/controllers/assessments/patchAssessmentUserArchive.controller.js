...
function isUserArchiveBlocked(user) {
...
function isTimeExpiredAt(expiryAt) {
    if (expiryAt == null)
        return false;
    try {
...
async function getAttestationExpiryForCotsVendor(assessmentId) {
    const [cots] = await db
        ...
        .limit(1);
    if (!cots?.attestationId)
        return null;
    const { rows } = await pool.query(`SELECT vsa.expiry_at FROM vendor_self_attestations vsa
     WHERE (vsa.vendor_self_attestation_id = $1::uuid OR vsa.id = $1::uuid) LIMIT 1`, [String(cots.attestationId)]);
 ...
 */
const patchAssessmentUserArchive = async (req, res) => {
    try {
