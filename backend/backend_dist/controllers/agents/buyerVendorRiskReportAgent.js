...
function buildRecommendationSummaryFromFullReport(executiveSummary, keyStrengths, areasForImprovement, recommendations) {
    const bullets = [];
    ...
    for (const s of keyStrengths.slice(0, 1)) {
        ...
        const t = s.trim();
        if (t)
            bullets.push(`Strength to leverage: ${t}`.slice(0, 280));
    }
    ...
    for (const r of recommendations.slice(0, 2)) {
        ...
        bullets.push(`${r.priority} — ${r.title}${tail ? `: ${tail}${r.description.length > 100 ? "…" : ""}` : ""}`.slice(0, 280));
    }
...
function defaultStructuredSections(vendorName, productName, hasAttestation, overallRiskScore, keyStrengths, areasForImprovement, riskAnalysis, recommendations, executiveSummary, ctx) {
    ...
    const bucketText = highDomains.length > 0
    ...
    const summary = buildRecommendationSummaryFromFullReport(executiveSummary, keyStrengths, areasForImprovement, recommendations);
    return {
        ...
        comparisonMatrix: [
            ...
            },
            {
                ...
                cells: [
                    {
                        vendorLabel: label,
                        highlight: "Cross-check integration, data handling, and governance expectations from the buyer assessment against vendor disclosures; gaps drive remediation items below.",
                    },
                ],
            },
            {
                ...
                cells: [
                    {
                        vendorLabel: label,
                        highlight: "Review certificates, hosting model, incident response, and residency options against regulatory and sensitivity requirements stated in the assessment.",
                    },
                ],
            },
        ],
...
function buildFallbackReport(vendorName, productName, hasAttestation) {
    const now = new Date().toISOString();
    const overallRiskScore = hasAttestation ? 75 : 55;
    const executiveSummaryBase = `This assessment evaluates ${vendorName}'s ${productName} against your stated requirements. ${hasAttestation
        ? "Vendor self-attestation data was available and informs this summary."
        : "A matching public vendor attestation was not found; strengthen due diligence with direct vendor evidence."} Review recommendations before implementation.`;
    ...
    const riskAnalysis = [
        {
            ...
            riskScope: "both",
            riskScore: 4,
            summary: "Vendor risk: confirm what the vendor attests for data handling, retention, and subprocessors. Buyer risk: your sensitivity and residency rules may require tighter controls than the vendor’s default.",
        },
        {
            domain: "Security Controls & Infrastructure",
            riskScope: "vendor",
            riskScore: 4,
            summary: "Vendor risk: request SOC 2, pen test summaries, and encryption posture. Buyer risk: your integration and identity model determines how much exposure you inherit in production.",
        },
        {
            domain: "Regulatory Compliance",
            riskScope: "buyer",
            riskScore: 5,
            summary: "Vendor risk: map certifications and compliance scope to the product. Buyer risk: your sector and obligations may exceed what the vendor’s attestations cover.",
        },
        {
            domain: "AI Governance & Ethics",
            riskScope: "buyer",
            riskScore: 5,
            summary: "Vendor risk: review documented model governance and acceptable use. Buyer risk: internal AI policies and oversight must close gaps for your use cases.",
        },
        {
            domain: "Operational Resilience",
            riskScope: "both",
            riskScore: 4,
            summary: "Vendor risk: validate SLA, uptime, and incident response. Buyer risk: your criticality and recovery objectives drive how much residual downtime risk is acceptable.",
        },
        {
            domain: "Vendor Financial Stability",
            riskScope: "vendor",
            riskScore: 3,
            summary: "Vendor risk: assess supplier viability and roadmap commitment. Buyer risk: long-term dependency may be unacceptable if procurement or continuity standards are strict.",
        },
        {
            domain: "Integration & Technical Architecture",
            riskScope: "buyer",
            riskScore: 5,
            summary: "Vendor risk: APIs, SSO, and hosting options from the product. Buyer risk: your stack and security architecture determine integration complexity and attack surface.",
        },
        {
            domain: "Model Reliability & Hallucination Risk",
            riskScope: "both",
            riskScore: 6,
            summary: "Vendor risk: transparency on model limits and monitoring. Buyer risk: high-stakes decisions amplify harm if outputs are wrong—design human review accordingly.",
        },
        {
            domain: "Vendor Lock-in & Portability",
            riskScope: "both",
            riskScore: 5,
            summary: "Vendor risk: export, APIs, and data portability terms. Buyer risk: your exit and multi-vendor strategy affects acceptable lock-in.",
        },
        {
            domain: "Third-Party & Supply Chain Risk",
            riskScope: "vendor",
            riskScore: 4,
            summary: "Vendor risk: sub-processors and cloud chain for this product. Buyer risk: your third-party risk program may require additional due diligence or contractual pass-through.",
        },
    ];
    const recommendations = [
        {
            ...
            description: "Define data classification, retention, and access rules aligned with the sensitivity level you selected in the assessment.",
            timeline: "Immediate",
        },
        {
            priority: "High",
            title: "Integration and security review",
            description: "Run a structured review of SSO, APIs, and logging against your integration and audit requirements.",
    ...
    const ctx = {};
    const structured = defaultStructuredSections(vendorName, productName, hasAttestation, overallRiskScore, keyStrengths, areasForImprovement, riskAnalysis, recommendations, executiveSummary, ctx);
    return {
        generatedAt: now,
        overallRiskScore,
...
function parseBuyerPriorities(raw) {
...
function parseRankedVendors(raw) {
...
function parseComparisonMatrix(raw) {
    if (!Array.isArray(raw) || raw.length === 0)
        return null;
    const out = [];
    for (const item of raw) {
...
function parseCapabilityGaps(raw) {
    if (!Array.isArray(raw) || raw.length === 0)
        return null;
    const out = [];
    for (const item of raw) {
...
function parseRecommendationSummary(raw) {
    if (!Array.isArray(raw) || raw.length === 0)
        return null;
    const bullets = raw.map((s) => String(s).slice(0, 500)).filter(Boolean);
    if (bullets.length === 0)
        return null;
    return bullets.slice(0, 4);
}
function normalizeRiskScope(v, fallback) {
    const s = String(v ?? "").toLowerCase().trim();
    if (s === "vendor" || s === "buyer" || s === "both")
        return s;
    return fallback;
}
function normalizeReport(raw, vendorName, productName, hasAttestation) {
    ...
    const recs = Array.isArray(raw.recommendations)
        ? raw.recommendations
            ...
            const pr = p === "High" || p === "Low" ? p : "Medium";
            return {
                priority: pr,
    ...
    const keyStrengths = Array.isArray(raw.keyStrengths)
        ? raw.keyStrengths.map((s) => String(s).slice(0, 400)).filter(Boolean)
        : fb.keyStrengths;
    const areasForImprovement = Array.isArray(raw.areasForImprovement)
        ? raw.areasForImprovement.map((s) => String(s).slice(0, 500)).filter(Boolean)
        : fb.areasForImprovement;
    const executiveSummary = ensureExecutiveSummaryHasTrustScore(String(raw.executiveSummary ?? fb.executiveSummary).slice(0, 4000), overallRiskScore);
    const recommendations = recs.length > 0 ? recs : fb.recommendations;
    const riskAnalysis = domains.length >= 5 ? domains : fb.riskAnalysis;
    const ctx = {};
    const defaults = defaultStructuredSections(vendorName, productName, hasAttestation, overallRiskScore, keyStrengths, areasForImprovement, riskAnalysis, recommendations, executiveSummary, ctx);
    ...
    const rankedEligibleVendors = parsedRanked !== null ? parsedRanked : defaults.rankedEligibleVendors;
    ...
    const implementationRiskScore = Number.isFinite(implementationRiskScoreRaw)
        ...
        : undefined;
    const implementationRiskClassification = raw.implementationRiskClassification != null
        ? String(raw.implementationRiskClassification).slice(0, 120)
        : undefined;
    const implementationRiskDecision = raw.implementationRiskDecision != null
        ? String(raw.implementationRiskDecision).slice(0, 120)
        : undefined;
    const implementationRiskRecommendedAction = raw.implementationRiskRecommendedAction != null
        ? String(raw.implementationRiskRecommendedAction).slice(0, 500)
        : undefined;
    const implementationRiskBreakdown = raw.implementationRiskBreakdown != null && typeof raw.implementationRiskBreakdown === "object"
        ? raw.implementationRiskBreakdown
        : undefined;
    return {
 ...
 */
export async function generateBuyerVendorRiskReport(buyerPayload, attestationRow, vendorName, productName) {
    const hasAttestation = attestationRow != null;
    const implementationRisk = calculateBuyerImplementationRiskScore(buyerPayload, attestationRow, vendorName, productName);
    const attestationSlice = attestationRow
        ? {
 ...
 */
export function enrichStoredBuyerVendorReport(report, vendorName, productName, ctx) {
    const hasAttestationHint = Array.isArray(report.keyStrengths) && report.keyStrengths.length >= 2;
    ...
    const priorities = parseBuyerPriorities(report.buyerPrioritiesAndWeights) ??
        defaultBuyerPrioritiesFromContext({
            ...ctx,
            regulatorySnippet: regSnip,
        });
    const preservedFramework = report.frameworkMapping != null && typeof report.frameworkMapping === "object"
