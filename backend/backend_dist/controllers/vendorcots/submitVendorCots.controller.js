...
function appendixCatalogRisksAndMitigations(top5) {
    if (!top5?.top5Risks?.length)
        return undefined;
    const items = [];
...
function appendixRiskMitigationActions(top5) {
    if (!top5?.top5Risks?.length)
        return undefined;
    const items = [];
    for (const r of top5.top5Risks) {
...
import { resolveFrameworkMappingRowsForAttestation, countSubstantiveFrameworkMappingRows, vendorCotsFrameworkMappingRowsFromAttestation, } from "../../services/frameworkMappingFromCompliance.js";
...
async function createCustomerRiskReport(assessmentId, orgIdStr, payloadCots, reviewedByUser) {
    const vendorAttestationId = payloadCots.vendor_attestation_id != null ? String(payloadCots.vendor_attestation_id).trim() : null;
    let productName = "";
    let attestationFrameworkSource = null;
    if (vendorAttestationId) {
        const [row] = await db
            ...
            .where(or(eq(vendorSelfAttestations.id, vendorAttestationId), eq(vendorSelfAttestations.vendor_self_attestation_id, vendorAttestationId)))
            .limit(1);
    ...
    const frameworkRowsFromDocuments = attestationFrameworkSource
        ...
        : [];
    const substantiveAttestationFrameworkCount = countSubstantiveFrameworkMappingRows(frameworkRowsFromDocuments);
    const frameworkMappingAttestationStatus = !vendorAttestationId || !attestationFrameworkSource
        ? "missing"
    ...
    const frameworkRowsForStoredReport = narrowVendorCotsFrameworkRowsToAiDomainControls(mergeVendorCotsAttestationRowsWithRegulatoryGaps(vendorCotsFrameworkMappingRowsFromAttestation(attestationFrameworkSource), payloadCots), payloadCots).map((r) => ({
    ...
    const report = {
        ...
        productFeatures: payloadCots.product_features != null
        ...
        regulatoryRequirements: payloadCots.regulatory_requirements != null
        ...
        customerSpecificRisks: payloadCots.customer_specific_risks != null
    ...
    let top5RisksWithMitigations = null;
    try {
    ...
    if (top5RisksWithMitigations) {
        report.dbTop5Risks = {
            ...
            mitigationsByRiskId: Object.fromEntries(Object.entries(top5RisksWithMitigations.mitigationsByRiskId).map(([rid, list]) => [
                rid,
                list.map((m) => ({
    ...
    const generated = await generateVendorCotsReport(payloadCots, top5RisksWithMitigations, frameworkRowsForStoredReport);
    if (generated) {
        if (generated.matchedRiskSummaries &&
            generated.matchedRiskSummaries.length > 0 &&
            report.dbTop5Risks != null &&
            typeof report.dbTop5Risks === "object") {
            const dbTop = report.dbTop5Risks;
            const rows = dbTop.top5Risks;
            if (Array.isArray(rows)) {
                const byId = new Map(generated.matchedRiskSummaries.map((m) => [
        ...
        if (generated.matchedMitigationSummaries &&
            ...
            if (mitMap && typeof mitMap === "object") {
                const mitigationIdByRiskAndName = new Map();
                ...
                for (const entry of generated.matchedMitigationSummaries) {
                    ...
                    if (!Array.isArray(list))
                        continue;
                    const idx = list.findIndex((m) => String(m.mitigation_action_id ?? "").trim() === mitigationId);
                    if (idx >= 0) {
...
const submitVendorCotsAssessment = async (req, res) => {
    try {
        ...
        const reviewedByUser = [firstName, lastName].filter(Boolean).join(" ") || userName || email || "—";
        const body = req.body ?? {};
