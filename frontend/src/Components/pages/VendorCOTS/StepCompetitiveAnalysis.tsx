...
const StepCompetitiveAnalysis = ({ data, formData, setFormData }: { data: Record<string, { label: string; required?: boolean; placeholder?: string }>; formData: Record<string, string>; setFormData: React.Dispatch<React.SetStateAction<Record<string, string>>> }) => {
  const update = (key: string) => (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    setFormData((prev) => ({ ...prev, [key]: e.target.value }));
  };
  return (
    <>
      <HeaderForVendor title_vendor="Competitive Analysis" className="header_for_vendor" />
      <div>
        ...
        <div className="form_fields_vendor">
          <FormField label={data[1].label} mandatory={data[1].required} tooltipText={data[1].placeholder}>
            <Input id={KEYS[1]} labelName="" type="textarea" name={KEYS[1]} value={formData[KEYS[1]] ?? ""} onChange={update(KEYS[1])} />
          </FormField>
        </div>
      </div>
    </>
